        ...
        @foreach($articles as $article)
            <div class="sticky hentry hentry-post blog-big group">
                ...
                <div class="meta group">
                    {{--modeli mej grac e kaper one to many, u ed kaperov el kstananq $article->user->name--}}
            ...
            {{--{{ $articles->links() }}--}}   {{-- paginate() functioan controlleri mej ogtagorceluc heto, view mej ete kanchenq links() functian kstacvi ashxatox pagination --}}
            <div class="general-pagination group">
                @if($articles->lastPage() > 1)

                    @if($articles->currentPage() != 1)
                        <a href="{{ $articles->url($articles->currentPage() - 1) }}">{{ Lang::get('ru.previous') }}</a>
                        @endif
                    ...
                    @for($i = 1; $i <= $articles->lastPage(); $i++)
                        ...
                        @if($articles->currentPage() != $articles->lastPage())
                            <a href="{{ $articles->url($articles->currentPage() + 1) }}">{{ Lang::get('ru.next') }}</a>
                            @endif
                    @endif
                @else
